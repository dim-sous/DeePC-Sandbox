"""Three-stage gate for v4.

Sparse QP form. Same checks as v2 plus L1 is now the default.
...
def stage_b(config: DeePCConfig) -> tuple[list[dict], dict]:
    ...
    metrics = compute_all_metrics(results, "v4")
...
def _generate_stress_plots(results_dir: pathlib.Path) -> None:
    ...
    fig.suptitle("v4 — Stress Test Results", fontsize=16, fontweight="bold")
    ...
    plt.savefig(results_dir / "v4_stress_tests.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def print_report(
    ...
    print()
    print("=" * 62)
    print("  v4  —  THREE-STAGE GATE REPORT")
    print("=" * 62)
...
def main() -> None:
    ...
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    report_path = RESULTS_DIR / "v4_gate_report.json"
