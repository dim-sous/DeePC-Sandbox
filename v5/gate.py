"""Three-stage gate for v5.

Robust DeePC with noise-adaptive regularization.
Same checks as v4 plus A11/A12 for noise estimator validation.

Usage (from repo root):
    uv run python -m v5.gate
"""
...
def stage_b(config: DeePCConfig) -> tuple[list[dict], dict]:
    ...
    metrics = compute_all_metrics(results, "v5")
...
def _generate_stress_plots(results_dir: pathlib.Path) -> None:
    ...
    fig.suptitle("v5 — Stress Test Results", fontsize=16, fontweight="bold")
    ...
    plt.savefig(results_dir / "v5_stress_tests.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def print_report(
    ...
    print()
    print("=" * 62)
    print("  v5  —  THREE-STAGE GATE REPORT")
    print("=" * 62)
...
def main() -> None:
    ...
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    report_path = RESULTS_DIR / "v5_gate_report.json"
