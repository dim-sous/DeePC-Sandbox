"""v5 — Robust DeePC with noise-adaptive regularization.

Builds on v4's sparse QP form. Regularization parameters (lambda_g,
lambda_y) adapt online based on estimated noise level, implementing
the Coulson/Lygeros/Dörfler (arXiv:1903.06804) insight that
regularization = Wasserstein robustness radius. Output constraints
are tightened proportionally to estimated noise.

Usage (from repo root):
    uv run python -m v5.main
    uv run python -m v5.main --scenario hard|circle|square|zigzag
"""
...
RESULTS_DIR = REPO_ROOT / "results" / "v5"
...
def run_deepc_simulation(
    ...
    # Noise estimator for adaptive regularization
    ...
    for k in range(config.sim_steps):
        ...
        # Update noise estimate and adapt regularization
...
def main() -> None:
    """Entry point."""
    parser = argparse.ArgumentParser(description="v5 DeePC simulation")
    parser.add_argument(
