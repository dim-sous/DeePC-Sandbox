"""Plotting utilities for v2 DeePC results."""
...
def _plot_impl(results: dict, config: DeePCConfig, save_dir: str) -> None:
    ...
    fig.suptitle("v2 — DeePC Trajectory Tracking", fontsize=13, fontweight="bold", y=0.97)
    ...
    # ── [2,0] Input Rates ─────────────────────────────────────────
    ax = axes[2, 0]
    ax.plot(t_du, du[:, 0], color=_C_STEER, linewidth=1.0, label="$\\Delta\\delta$ [rad/step]")
    ax.axhline(-config.d_delta_max, color=_C_STEER, linestyle=":", linewidth=0.7, alpha=0.5)
    ax.axhline(config.d_delta_max, color=_C_STEER, linestyle=":", linewidth=0.7, alpha=0.5)
    ax.set_ylabel("$\\Delta\\delta$ [rad/step]", color=_C_STEER)
    ax.tick_params(axis="y", labelcolor=_C_STEER)

    ax2 = ax.twinx()
    ax2.plot(t_du, du[:, 1], color=_C_ACCEL, linewidth=1.0, label="$\\Delta a$ [m/s$^2$/step]")
    ax2.axhline(-config.da_max, color=_C_ACCEL, linestyle=":", linewidth=0.7, alpha=0.5)
    ax2.axhline(config.da_max, color=_C_ACCEL, linestyle=":", linewidth=0.7, alpha=0.5)
    ax2.set_ylabel("$\\Delta a$ [m/s$^2$/step]", color=_C_ACCEL)
    ax2.tick_params(axis="y", labelcolor=_C_ACCEL)
    ax.set_xlabel("Time [s]")
    ax.set_title("Input Rates + Constraint Bands")
    lines1, labels1 = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines1 + lines2, labels1 + labels2, loc="upper right")
    ...
    ax = axes[2, 1]
    solve_times = np.array(results["solve_times"]) * 1000  # ms
    t_solve = times[config.Tini : config.Tini + len(solve_times)]
    ax.plot(t_solve, solve_times, color="#9467bd", linewidth=0.8)
    ax.axhline(np.mean(solve_times), color="#9467bd", linestyle=":", linewidth=0.8,
    ...
    path = f"{save_dir}/v2_results.png"
    fig.savefig(path)
    plt.close(fig)
