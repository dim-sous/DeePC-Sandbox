"""Three-stage gate for v2.

Extends v1 gate with checks for:
  - Input rate constraints (A8, B7)
  - Output constraint slack (A9, B6)
  - L1 regularization smoke test (A10)

Usage (from repo root):
    uv run python -m v2.gate
"""
...
def stage_b(config: DeePCConfig) -> tuple[list[dict], dict]:
    ...
    metrics = compute_all_metrics(results, "v2")
...
def _generate_stress_plots(results_dir: pathlib.Path) -> None:
    ...
    fig.suptitle("v2 — Stress Test Results", fontsize=16, fontweight="bold")
    ...
    plt.savefig(results_dir / "v2_stress_tests.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def print_report(
    ...
    print()
    print("=" * 62)
    print("  v2  —  THREE-STAGE GATE REPORT")
    print("=" * 62)
...
def main() -> None:
    ...
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    report_path = RESULTS_DIR / "v2_gate_report.json"
