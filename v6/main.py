"""v6 — Online sliding Hankel window DeePC.

Builds on v5's noise-adaptive regularization. The Hankel matrices
are updated each control step with closed-loop I/O data via a
fixed-size sliding window (arXiv:2407.16066). The implicit model
adapts to the current operating regime online.

Usage (from repo root):
    uv run python -m v6.main
    uv run python -m v6.main --scenario hard|circle|square|zigzag
"""
...
def run_deepc_simulation(
    ...
    for k in range(config.sim_steps):
        ...
        # Online Hankel update: after warmup, only when tracking is good
        if k >= config.hankel_warmup_steps:
            noise_std = noise_est.get_noise_std()
            mean_residual = float(np.mean(noise_std))
            if mean_residual < 1.0:  # only add good-quality data
...
def main() -> None:
    """Entry point."""
    parser = argparse.ArgumentParser(description="v6 DeePC simulation")
    parser.add_argument(
