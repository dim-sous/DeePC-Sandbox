"""Centralized configuration for the v3 DeePC experiment."""
...
@dataclass
class DeePCConfig:
    """All tunable parameters for the v3 DeePC experiment."""
    ...
    input_amplitude_a: float = 3.0  # acceleration excitation amplitude [m/s^2]
    ...
    Q_diag: list[float] = field(default_factory=lambda: [100.0, 100.0, 1.0])
    R_diag: list[float] = field(default_factory=lambda: [0.1, 0.1])

    # --- Regularization ---
    lambda_g: float = 10.0  # penalty on g
    lambda_y: float = 1e4  # penalty on output slack sigma_y
    reg_norm_g: str = "L2"  # "L1" or "L2" for g regularization
    reg_norm_sigma_y: str = "L2"  # "L1" or "L2" for sigma_y regularization
    ...
    da_max: float = 1.5  # max acceleration rate (jerk) [m/s^2/step]

    # --- Output constraints (soft) ---
    # per-channel bounds; use inf for unconstrained channels
    y_lb: list[float] = field(default_factory=lambda: [float("-inf"), float("-inf"), 2.0])
    y_ub: list[float] = field(default_factory=lambda: [float("inf"), float("inf"), 8.0])
