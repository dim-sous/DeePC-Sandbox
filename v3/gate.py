"""Three-stage gate for v3.

Inherits v2 checks. Output scaling is internal to the controller
so gate checks are unchanged.

Usage (from repo root):
    uv run python -m v3.gate
"""
...
def stage_a(config: DeePCConfig, u_data: np.ndarray, y_data: np.ndarray) -> list[dict]:
    ...
    # A10: L1 regularization smoke test (L1/L2 only — full L1/L1 is unreliable with OSQP)
    l1_cfg = DeePCConfig(reg_norm_g="L1", reg_norm_sigma_y="L2")
    ...
    checks.append({"name": "A10 L1/L2 regularization solves to optimality",
...
def _test_high_noise() -> bool:
    ...
    _plot_data["high_noise"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C1: High Noise (10x)"}
...
def _test_aggressive_reference() -> bool:
    ...
    rmse = _compute_rmse_position(results)
    optimal_pct = 100.0 * sum(1 for s in results["statuses"] if "optimal" in s) / len(results["statuses"])
    _plot_data["aggressive_ref"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C2: Aggressive Reference"}
...
def _test_reduced_data() -> bool:
    ...
    rmse = _compute_rmse_position(results)
    optimal_pct = 100.0 * sum(1 for s in results["statuses"] if "optimal" in s) / len(results["statuses"])
    _plot_data["reduced_data"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C3: Reduced Data (T=50)"}
...
def _test_tight_constraints() -> bool:
    ...
    _plot_data["tight_constraints"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C4: Tight Constraints"}
...
def _test_nonlinear_regime() -> bool:
    ...
    rmse = _compute_rmse_position(results)
    optimal_pct = 100.0 * sum(1 for s in results["statuses"] if "optimal" in s) / len(results["statuses"])
    _plot_data["nonlinear"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C5: Nonlinear Regime"}
    return rmse < 20.0 and optimal_pct > 30.0
...
def _test_step_reference() -> bool:
    ...
    _plot_data["step_ref"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C6: Step Reference"}
...
def _test_disturbance_rejection() -> bool:
    ...
    rmse = _compute_rmse_position(results)
    _plot_data["disturbance"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C7: Disturbance Rejection"}
    return rmse < 5.0
...
def _test_long_horizon() -> bool:
    ...
    _plot_data["long_horizon"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C8: Long Horizon (200 steps)"}
...
def _test_rate_constrained_aggressive() -> bool:
    ...
    rate_steer = bool(np.all(np.abs(du[:, 0]) <= config.d_delta_max + 1e-4))
    rate_accel = bool(np.all(np.abs(du[:, 1]) <= config.da_max + 1e-4))
    _plot_data["rate_aggressive"] = {"y_hist": results["y_history"], "y_ref": results["y_ref_history"], "title": "C9: Rate-Constrained Aggressive"}
...
def _generate_stress_plots(results_dir: pathlib.Path) -> None:
    ...
    fig.suptitle("v3 — Stress Test Results", fontsize=16, fontweight="bold")
    ...
    plt.savefig(results_dir / "v3_stress_tests.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def print_report(
    ...
    print()
    print("=" * 62)
    print("  v3  —  THREE-STAGE GATE REPORT")
    print("=" * 62)
...
def main() -> None:
    ...
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    report_path = RESULTS_DIR / "v3_gate_report.json"
