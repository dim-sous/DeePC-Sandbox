"""Common stress test configurations shared across all versions.

Each function returns a dict of parameter overrides to pass to the
version's DeePCConfig.  Using a shared definition ensures the stress
test *scenarios* are identical even when version defaults differ.
"""

# Common base overrides applied to every stress test config.
# These pin the data-collection and scenario parameters that vary
# across versions so the comparison is apples-to-apples.
_COMMON = dict(
    T_data=200,
